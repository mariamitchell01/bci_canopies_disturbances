...
cat23f <- lidR::catalog("D:/TroubleshootingV/pointclouds/BCI_whole_2023_06_19_medium.las")
...
soils <- rgdal::readOGR("D:/TroubleshootingV/Data_Ancillary/BCI_Soils/BCI_Soils.shp")
...
write.csv(gridInfo, row.names = F, file = ("D:/TroubleshootingV/Data_Ancillary/gridInfo.csv"))
...
for(i in 1:dim(gridInfo)[1]){
  ...
  if(length(data@data$X)>0){
    lidR::writeLAS(data, file=paste0("D:/TroubleshootingV/tiles/BCI23FTiles/BCI23_",i,".laz"))
  }
...
for(i in 1:dim(gridInfo)[1]){
  ...
  if(length(data@data$X)>0){
    lidR::writeLAS(data, file=paste0("D:/TroubleshootingV/tiles/BCI23FTiles_dec/BCI23d_",i,".laz"))
  }
...
for(i in 1:dim(gridInfo)[1]){
  file.rename(from = list.files("D:/TroubleshootingV/tiles/BCI23Tiles_dec/",
                                full.names = T, pattern = paste0("BCI23d_",i,"_REG")),
              to = paste0("D:/TroubleshootingV/tiles/BCI23Tiles_dec/BCI23mat_",i,".txt"))
}
...
path2 <- "D:/TroubleshootingV/tiles"
...
for (i in 1:dim(gridInfo)[1]) {
  data <- lidR::clip_rectangle(
    las = lidR::readLAS(paste0("D:/TroubleshootingV/tiles/BCI23Tiles_alignedto22Full/BCI23af_", as.character(i), ".las")),
    xleft = gridInfo$xmin[i],
    xright = gridInfo$xmax[i],
    ybottom = gridInfo$ymin[i],
    ytop = gridInfo$ymax[i]
  ...
  if (length(data@data$X) > 0) {
    lidR::writeLAS(
      las = data,
      file = paste0("D:/TroubleshootingV/tiles/BCI23Tiles_alignedto22Trim/BCI23at_", as.character(i), ".laz")
    )
  }
}
